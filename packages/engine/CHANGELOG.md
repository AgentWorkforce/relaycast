...
- `sweepTimedOutInvocations` accepts `handlerUnreachableTtlMs` and `completionDeps` options and `@relaycast/engine/node-invocations` exports `ACTION_HANDLER_UNREACHABLE_TTL_MS`: an open agent-handled invocation whose handler connection stays continuously unreachable for the TTL is failed with `handler_unavailable`, emitting `action.failed` to the caller when completion deps are provided. The unreachability clock starts at the sweep's first unreachable observation and resets on recovery (new D1 migration `0032_invocation_handler_unreachable.sql` adds `action_invocations.handler_unreachable_since`), so a brief handler restart never kills an in-flight invocation regardless of its age.
...
- `POST /v1/actions` re-registers an existing action name as an idempotent refresh (200) of its description, handler, schemas, `available_to`, and `is_active` — including moving the handler to a new agent identity — instead of surfacing the unique index as a 500 `internal_error`; residual registration races return 409 `action_name_conflict`. A refresh that moves the handler to a different agent fails invocations still in flight toward the previous handler (`handler_unavailable`) with `action.failed` to their callers, and `DELETE /v1/actions/:name` likewise fails the action's open invocations (`action_deleted`) instead of leaving them retrying against a deleted action.

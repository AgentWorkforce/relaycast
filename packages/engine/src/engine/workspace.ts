...
type CreateWorkspaceOptions =
  ...
  | {
       ...
       * binding. It MUST be an unpredictable value with at least 122 bits
       * of entropy (the randomness in a v4 UUID; equivalently 16+
       * cryptographically random bytes, hex- or base64url-encoded) drawn
       * from a CSPRNG, never a derived or guessable value such as a job id,
       * timestamp, or counter. `createWorkspace` enforces a minimum length
       * (`MIN_BOOTSTRAP_IDEMPOTENCY_KEY_LENGTH`) as a structural floor, but
       * cannot verify true randomness — callers are responsible for
       * generating the key with a CSPRNG.
       */
...
/**
 * Structural entropy floor for an anonymous bootstrap Idempotency-Key
 * (relaycast#379): the shortest length that can hold >=122 bits of entropy
 * even under the least entropy-dense encoding a caller is realistically
 * expected to use (hex, 4 bits/char — 122/4 = 30.5, rounded up). 122 bits
 * matches the randomness in a v4 UUID, so "at least as unpredictable as a
 * fresh UUIDv4" is the plain-language version of this contract. This is a
 * necessary, not sufficient, check: a 32-character key of a single repeated
 * character passes the length floor but is not unpredictable. The server
 * has no way to verify true randomness from a string alone — callers MUST
 * generate the key with a CSPRNG (see the idempotencyKey doc comment
 * above). This floor only applies to the anonymous bootstrap path; an
 * owner-scoped idempotency key is protected by the owner's API key instead.
 */
...
export async function createWorkspace(
  ...
  // part of what authorizes recovering a binding — so it must itself carry
  // enough entropy that no one could feasibly guess it, not just be unique.
  // This is a structural floor (length only), checked before any secret or
  // database work; it cannot prove the key was actually drawn from a CSPRNG.
  if (bootstrapIdempotency && idempotencyKey!.length < MIN_BOOTSTRAP_IDEMPOTENCY_KEY_LENGTH) {
    throw codedError(
      `Anonymous Idempotency-Key must be at least ${MIN_BOOTSTRAP_IDEMPOTENCY_KEY_LENGTH} characters, generated by a CSPRNG with at least 122 bits of entropy (e.g. a v4 UUID or 16+ random bytes hex/base64url-encoded)`,

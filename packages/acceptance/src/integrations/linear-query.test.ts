// @route POST /api/v1/linear/query
...
const configuredBody = envBody("ACCEPTANCE_LINEAR_QUERY_BODY");

describe("/api/v1/linear/query", () => {
  it("rejects requests without a bearer token", async () => {
    const response = await requestApi("/api/v1/linear/query", {
      method: "POST",
      json: {},
    });
  ...
  (hasSpecialistAuth() ? it : it.skip)(
    "returns not_found when the workspace has no Linear integration",
    async () => {
      const response = await requestApi("/api/v1/linear/query", {
        method: "POST",
        auth: "specialist",
      ...
      const body = await parseJson(response, errorSchema);
      expect(body.error).toBe("Linear workspace integration was not found.");
    },
  );

  (hasSpecialistAuth() && configuredBody ? it : it.skip)(
    "returns the configured Linear query response",
    async () => {
      const response = await requestApi("/api/v1/linear/query", {
        method: "POST",
        auth: "specialist",

...
function request(query = ""): NextRequest {
  return new NextRequest(
    `https://cloud.test/api/v1/workspaces/${WORKSPACE_ID}/reflex/outcomes${query}`,
  );
}
...
describe("GET /api/v1/workspaces/[workspaceId]/reflex/outcomes", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.fetchRelayhistoryReflex.mockResolvedValue(
      Response.json({
        outcomes: [
          { id: "outcome_1", label: "Resolved escalation", status: "success" },
        ],
      }),
    );
  });
  ...
  it("returns proxied JSON and forwards only whitelisted query parameters", async () => {
    const response = await GET(
      request("?since=2026-06-01T00%3A00%3A00.000Z&limit=5&scope=ignored"),
      context(),
    );
    ...
    expect(mocks.fetchRelayhistoryReflex).toHaveBeenCalledWith(
      "/v1/reflex/outcomes?since=2026-06-01T00%3A00%3A00.000Z&limit=5",
      {
        userId: USER_ID,
        orgId: ORG_ID,
        workspaceId: WORKSPACE_ID,

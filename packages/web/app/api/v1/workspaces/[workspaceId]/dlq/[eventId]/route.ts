...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const response = await fetch(
      `${access.gatewayBaseUrl.replace(/\/+$/, "")}/v1/workspaces/${encodeURIComponent(access.relayWorkspaceId)}/dlq/${encodeURIComponent(eventId)}`,
      {
        cache: "no-store",
        headers: {
    ...
    console.error("Workspace DLQ inspect failed:", error instanceof Error ? error.message : String(error));

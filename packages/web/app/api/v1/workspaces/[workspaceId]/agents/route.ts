...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const access = await resolveWorkspaceGatewayAccess({
      ...
      workspaceId,
      agentName: "cloud-dashboard-agents",
      requestedScopes: ["relayfile:fs:read:*"],
    });
    const response = await fetch(
      `${access.gatewayBaseUrl.replace(/\/+$/, "")}/v1/workspaces/${encodeURIComponent(access.relayWorkspaceId)}/agents`,
      {
        cache: "no-store",
        headers: {

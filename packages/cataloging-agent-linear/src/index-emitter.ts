...
function toIndexRow(entry: DirectoryIndexEntryInput): DirectoryIndexRow | null {
  ...
  const state = readLinearState(parsed?.state);
  const identifier = readString(parsed?.identifier);
...
function readLinearState(value: unknown): string | null {
  if (typeof value === "string") {
  ...
  return readString(value.name) ?? readString(value.type);
}

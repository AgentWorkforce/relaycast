...
export class HttpClient {
  ...
  async requestRaw<T>(
    ...
    while (true) {
      ...
      const parsedData = options?.schema ? options.schema.parse(json) : json;
      return camelizeKeys(parsedData) as Camelize<T>;
    }
  }
  ...
  getRaw<T>(path: string, query?: Record<string, string>, options?: RequestOptions): Promise<Camelize<T>> {
    return this.requestRaw<T>('GET', path, undefined, query, options);
  }

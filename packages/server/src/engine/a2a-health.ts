...
async function pingExternalAgent(externalUrl: string): Promise<boolean> {
  try {
    const response = await fetch(buildAgentCardUrl(externalUrl), {
      method: 'GET',
...
async function updateA2aAgentHealth(
  ...
  await db.$client
    .prepare(`
      UPDATE a2a_agents
      SET status = ?, health_failures = ?, last_health = unixepoch(), updated_at = unixepoch()
      WHERE id = ?
    `)
    .bind(status, healthFailures, agentId)
    .run();
}

...
async function drainEntry(entry: tar.ReadEntry): Promise<void> {
  ...
  return await new Promise<WalkedEntry>((resolve, reject) => {
    entry.on("data", (chunk: Buffer | Uint8Array) => {
      ...
      if (!knownBinary && sniffBytes < MAX_BINARY_SNIFF_BYTES) {
        const sniffChunk = buffer.subarray(0, MAX_BINARY_SNIFF_BYTES - sniffBytes);
    ...
    entry.once("end", () => {
      ...
      const isBinary =
        knownBinary ||
        Buffer.concat(sniffChunks).includes(0) ||
        (!ignored && !tooLarge && !isValidUtf8(content));

      if (ignored) {
        resolve({ repoPath, content: Buffer.alloc(0), isBinary, size, skipped: "ignored" });
        return;
      }

      if (tooLarge) {
        resolve({ repoPath, content: Buffer.alloc(0), isBinary, size, skipped: "too-large" });
        return;
      }

      resolve({ repoPath, content, isBinary, size });
    });
  });
}

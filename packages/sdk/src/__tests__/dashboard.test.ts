...
describe('Relay dashboard methods', () => {
  ...
  it('stats() calls GET /v1/workspace/stats', async () => {
    ...
    expect(url).toBe('https://api.agentrelay.dev/v1/workspace/stats');
  ...
  it('activity() calls GET /v1/activity without params', async () => {
    ...
    expect(url).toBe('https://api.agentrelay.dev/v1/activity');
  });
  ...
  it('activity(5) calls GET /v1/activity?limit=5', async () => {
    ...
    const [url] = mockFetch.mock.calls[0]!;
    expect(url).toBe('https://api.agentrelay.dev/v1/activity?limit=5');
  });
  ...
  it('allDmConversations() calls GET /v1/dm/conversations/all', async () => {
    ...
    const [url, init] = mockFetch.mock.calls[0]!;
    expect(url).toBe('https://api.agentrelay.dev/v1/dm/conversations/all');
    expect(init.method).toBe('GET');
  });
  ...
  it('agents.rotateToken() calls POST /v1/agents/:name/rotate-token', async () => {
    ...
    const [url, init] = mockFetch.mock.calls[0]!;
    expect(url).toBe('https://api.agentrelay.dev/v1/agents/TestBot/rotate-token');
  ...
  it('agents.rotateToken() URL-encodes the agent name', async () => {
    ...
    const [url] = mockFetch.mock.calls[0]!;
    expect(url).toBe('https://api.agentrelay.dev/v1/agents/a%2Fb/rotate-token');
  });
});

...
The image contains `@relaycast/engine` **8.10.0** on Node 22.23.2 and stores all
...
The version command must print `8.10.0`, and Compose should eventually report
...
Agent-card discovery works on the standard path from engine 8.10.0. A
...
image is on 8.10.0 or later before looking anywhere else.

In engine 8.10.0, `POST /v1/workspaces` is intentionally unauthenticated for

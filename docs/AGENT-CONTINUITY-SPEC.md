...
4. Use `--fresh` flag on spawn to skip continuity injection.

### Use Cases

1. **Agent release + re-spawn** — Agent is released, new agent spawned with same name picks up context
2. **Agent crash recovery** — Agent process dies unexpectedly, broker auto-restarts with continuity injected
3. **Session handoff** — End of day, new session next morning continues where previous left off
4. **Model switching** — Agent switched from sonnet to opus mid-task, continuity preserves context across the switch
5. **Scaling** — Agent released due to resource limits, re-spawned when capacity available
...
- Add `--fresh` flag to spawn to skip injection
...
- Continuity is fully automatic — zero manual action required

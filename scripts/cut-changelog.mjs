...
if (!version || !/^\d+\.\d+\.\d+(?:-[\w.]+)?$/.test(version)) {
  console.error(
    "usage: cut-changelog.mjs --version <x.y.z> [--date <yyyy-mm-dd>] [--from-tag <tag>] [--dry-run]",
  );
  process.exit(1);
}
...
function fallbackBody(fromTag) {
  if (!fromTag) return "";
  const subjects = git(
    `git log ${fromTag}..HEAD --no-merges --pretty=format:%s`,
  )
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
...
if (!fromTag)
  warn("no previous stable tag found; commit-subject fallback disabled");
...
if (fromTag && rootWasCut) {
  ...
  if (!dryRun) writeFileSync("CHANGELOG.md", withComparisons);
  console.log(`CHANGELOG.md: comparison links now start at v${version}`);
}
...
if (fromTag) {
  ...
  for (const { file, level } of levels) {
    if (rank[level] > rank[actual]) {
      warn(
        `${file} pending entries are marked ${level} but v${version} is a ${actual} bump`,
      );
    }
  }
}

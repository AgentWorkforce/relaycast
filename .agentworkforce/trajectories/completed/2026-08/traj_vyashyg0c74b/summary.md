...
- Action invocation now carries one stable SDK key through transport retries; the engine atomically claims a deterministic invocation row with immutable handler identity before provider dispatch, replays that invocation, rejects payload reuse, and suppresses duplicate provider, webhook, telemetry, and observer effects. Focused and full Node 22 suites are green (engine 680/680, SDK 436/436), builds/typecheck and package linters pass.

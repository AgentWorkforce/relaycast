...
- Action invocation now carries one stable SDK key through transport retries; the engine atomically claims a deterministic invocation row with immutable handler and node identity before provider dispatch, waits for native-spawn placement before replaying, rejects payload reuse, and suppresses duplicate provider, webhook, telemetry, and observer effects. Focused and full Node 22 suites are green (engine 681/681, SDK 436/436), builds/typecheck and package linters pass.

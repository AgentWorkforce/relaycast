...
- CodeRabbit's atomicity finding replaced the initial post-dispatch KV record with a deterministic action_invocations primary-key claim inserted before dispatch. Concurrent keyed spawn and registered-action tests now prove one row and one provider frame even when KV writes fail; immutable handler identity survives action takeover, payload comparison is canonical, and replay-only webhook, telemetry, observer, placement, and capacity effects are suppressed. Node 22 validation is green: engine 680/680, SDK 436/436, focused concurrency suites, engine build/typecheck/lint, and SDK build/lint.

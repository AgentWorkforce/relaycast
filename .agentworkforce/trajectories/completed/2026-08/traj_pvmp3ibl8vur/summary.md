...
- CodeRabbit's atomicity finding replaced the initial post-dispatch KV record with a deterministic action_invocations primary-key claim inserted before dispatch. Concurrent keyed spawn and registered-action tests prove one row and one provider frame even when KV writes fail; every keyed replay waits for a durable dispatch or terminal outcome, and immutable handler/node identity survives action takeover, native-spawn races, and local or concurrent release replay. Payload comparison is canonical, and replay-only webhook, telemetry, observer, placement, and capacity effects are suppressed.

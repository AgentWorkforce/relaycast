...
> **Completed:** September 6, 2026 at 12:13 PM
...
**Attribution range:** `8e36b742ced89d5e2d4be0866a7f641b31e0acfb..6aef2748c46a3401e5f1077cade3c32dcd10d332`
...
- On combined product head `6aef2748c46a3401e5f1077cade3c32dcd10d332`, this trajectory reran focused engine regressions (4 files / 107 tests), root lint (13/13), root test (18/18, including the full engine suite: 70 files / 790 tests), and root build (9/9) after restacking once on current main. Six latest behaviors were reproduced on test-only head `2f6a6afabf2864b4d27283f5e0fb41737aed3656`: agent-hosted acceptance lacked a durable route and generation before send; migration 0047 left historical accepted registered actions unmarked; an explicit tombstone matched only invocation id rather than its node/provider/name tuple; registered node-action acceptance trusted a mutable dispatched capability rather than the invocation action; timeout recovery cleared an accepted pruned route; and an ID-less stale worker could consume a same-tuple replacement reservation. They pass through socket-owner acceptance CAS for both action hosts, selective historical backfill, immutable invocation-action validation, accepted-generation pinning through prune/takeover recovery, exact tombstone tuple correlation, and fail-closed handling of ambiguous ID-less same-tuple registration. Earlier red regressions prove guarded release dispatch and completion, replay durability, retry/drain/reconciliation behavior, registration rollback, migration capacity accounting, exact action-generation handoff, and stale concurrent loser isolation.
- On product head `2951bcb435a578c4bfc35c4b9b62c4455857a052`, this trajectory ran the TypeScript SDK (22 files / 441 tests), types package (7 files / 209 tests), Rust SDK (103 tests plus 5 doc tests), and Rust library clippy. Those package code and test files are unchanged through combined product head `6aef2748c46a3401e5f1077cade3c32dcd10d332`; later engine-only commits harden lifecycle routing and raise no SDK surface changes.

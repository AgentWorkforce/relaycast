...
- Registered node-action dispatch transfers identity to a live same-name fallback while atomically claiming its route and attempt; a second retry cannot duplicate the frame, stale failure cannot clobber a newer attempt, and pruning after provider acceptance leaves route-owned completion intact.

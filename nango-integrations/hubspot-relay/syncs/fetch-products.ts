...
const PRODUCT_OBJECT_TYPE_ID = '0-7';
...
function toProductRecord(product: z.infer<typeof ProductApiSchema>): z.infer<typeof ProductSchema> {
    return {
        id: product.id,
...
async function fetchProductById(client: HubspotClient, productId: string): Promise<z.infer<typeof ProductSchema> | null> {
    try {
        // https://developers.hubspot.com/docs/api-reference/crm-products-v3/basic/get-crm-v3-objects-products-productId
        const response = await client.get({
            endpoint: `/crm/v3/objects/products/${productId}`,
            params: {
        ...
        return toProductRecord(ProductApiSchema.parse(response.data));
    } catch {
        return null;
    }
}
...
const sync = createSync({
    ...
    exec: async (nango) => {
        ...
        if (shouldUseInitialListSync) {
            ...
            while (hasMore) {
                ...
                const records = products.map((product) => toProductRecord(product));
    ...
    onWebhook: async (nango, payload) => {
        const { upsertIds, deleteIds } = extractHubspotWebhookObjectIds(payload, PRODUCT_OBJECT_TYPE_ID);

        if (deleteIds.length > 0) {
            await nango.batchDelete(deleteIds.map((id) => ({ id })), 'Product');
        }

        const records: Array<z.infer<typeof ProductSchema>> = [];
        for (const id of upsertIds) {
            const record = await fetchProductById(nango, id);
            if (record) {
                records.push(record);
            }
        }

// HubSpot sync record shapes — Nango-sandbox mirror.
//
// CANONICAL SOURCE: packages/core/src/sync/hubspot-record-shapes.ts
//
// Nango's sandbox cannot resolve `@cloud/core`, so this file mirrors the
// shared record builders and adds zod schemas for sync model validation.
...
export const HubSpotRawObjectSchema = z.object({
    id: z.string(),
    properties: z.record(z.string(), z.string().nullish()).nullish(),
...
export type HubSpotContactRecord = z.infer<typeof HubSpotContactSchema>;
export type HubSpotCompanyRecord = z.infer<typeof HubSpotCompanySchema>;
export type HubSpotDealRecord = z.infer<typeof HubSpotDealSchema>;
export type HubSpotTicketRecord = z.infer<typeof HubSpotTicketSchema>;
export type HubSpotRawObject = z.infer<typeof HubSpotRawObjectSchema>;
...
export const CONTACT_PROPERTIES = "firstname,lastname,email,phone,jobtitle,company,createdate,lastmodifieddate";
export const COMPANY_PROPERTIES = "name,domain,industry,city,state,country,phone,website,description,createdate,hs_lastmodifieddate";
export const DEAL_PROPERTIES = "dealname,amount,closedate,dealstage,hubspot_owner_id,description,hs_lastmodifieddate";
...
function isMatchingObject(event: HubSpotWebhookEvent, objectTypeId: string): boolean {
...
export function buildHubSpotContactRecord(contact: HubSpotRawObject): HubSpotContactRecord {
    return {
        id: contact.id,
...
export function buildHubSpotCompanyRecord(company: HubSpotRawObject): HubSpotCompanyRecord {
...
async function fetchAssociatedIds(client: HubSpotAssociationClient, dealId: string, association: "companies" | "contacts"): Promise<string[]> {
    try {
...
export async function buildHubSpotDealRecord(deal: HubSpotRawObject, client?: HubSpotAssociationClient): Promise<HubSpotDealRecord> {
    ...
    const companyIds =
        companyIdsFromPayload.length > 0 || !client ? companyIdsFromPayload : await fetchAssociatedIds(client, deal.id, "companies");
    const contactIds =
        contactIdsFromPayload.length > 0 || !client ? contactIdsFromPayload : await fetchAssociatedIds(client, deal.id, "contacts");
...
export function buildHubSpotTicketRecord(ticket: HubSpotRawObject): HubSpotTicketRecord {
    return {
        id: ticket.id,

...
function dockerAvailable() {
  return (
    sh("docker", ["version", "--format", "{{.Server.Version}}"]).status === 0
  );
}
...
function publishedPort(name) {
  const published = sh("docker", ["port", name, "8787/tcp"]);
  assert.equal(
    published.status,
    0,
    `docker port failed for ${name}: ${published.stderr}`,
  );
  const match = published.stdout.trim().match(/127\.0\.0\.1:(\d+)$/);
  assert.ok(
    match,
    `docker returned an unexpected loopback port mapping for ${name}`,
  );
...
function startContainer({ name, volumeDir, secret }) {
  ...
  const args = [
    ...
    "--name",
    name,
    // Let Docker allocate an unused loopback port. Fixed host ports make two
  ...
  assert.equal(
    result.status,
    0,
    `docker run failed for ${name}: ${result.stderr}`,
  );
  return publishedPort(name);
}
...
function stopContainer(name) {
  ...
  assert.equal(
    result.status,
    0,
    `docker stop failed for ${name}: ${result.stderr}`,
  );
}
...
function startExistingContainer(name) {
  ...
  assert.equal(
    result.status,
    0,
    `docker start failed for ${name}: ${result.stderr}`,
  );
  // Docker Desktop can assign a new ephemeral host port when a stopped
  // container is restarted. Re-resolve the actual mapping before polling.
  return publishedPort(name);
}

function containerLogs(name) {
  return (
    sh("docker", ["logs", name]).stdout + sh("docker", ["logs", name]).stderr
  );
}
...
async function createWorkspace(
  port,
  { name, idempotencyKey, secret, authorization },
) {
...
describe(
  "self-host image integration (real docker build + real container)",
  { skip: !dockerOk && "docker is not available" },
  () => {
    before(async () => {
    ...
    test("explicit invalid Authorization is rejected with 401, not downgraded to anonymous bootstrap", async () => {
      ...
      const { status, body } = await createWorkspace(primaryPort, {
    ...
    test("the bootstrap secret never appears in captured container logs", () => {
      ...
      for (const name of [PRIMARY_CONTAINER, SECONDARY_CONTAINER]) {
        ...
        assert.equal(
          leaked,
          false,
          `container "${name}" logged the bootstrap secret (value redacted from this message)`,
        );
      }
    });
  },
